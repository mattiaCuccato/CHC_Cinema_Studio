...
export default {
  ...
  async mounted() {
    if (this.$route.name == "films_edit") {
      ...
      let response = await axios.get("http://localhost:3000/films/" + filmId);
    ...
    async saveFilm() {
      ...
      if (this.isEdit) {
        response = await axios.put(
          "http://localhost:3000/films/" + this.$route.params.id,
          this.newFilm
        );
      } else {
        response = await axios.post("http://localhost:3000/films", this.newFilm);
      }
